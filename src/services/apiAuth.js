...
export async function updatePublicUser(avatar_url, id) {
...
export async function UploadReview({review,movieId}){
  ...
  const { data, error } = await supabase.auth.updateUser({data:{ reviews:[...updatedReviews,{review,movieId}]}});
  if (error) throw new Error(error.message);
  updatePublicreview([...updatedReviews,{review,movieId}],user.id)
  return data
}
